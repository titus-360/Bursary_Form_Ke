...
class BursaryControllerTest {
    ...
    fun `update bursary application - success`()  {
        val bursaryApplication = createBursaryApplication()
        val id = 1L

        `when`(bursaryService.updateBursaryApplication(bursaryApplication)).thenReturn(bursaryApplication)

        val response: ResponseEntity<BursaryApplication> = bursaryController.updateBursary(id, bursaryApplication)

        assertEquals(bursaryApplication, response.body)
        assertEquals(200, response.statusCode.value())
    }
